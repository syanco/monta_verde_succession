...
Finally, we'll simplify the migratory guild data to only include 3 groups: "resident", "migratory", or "nomadic".
...
The actual fixed effect coefficient estimates are also a bit confusing in their trend, but at least they are larger than their standard error (except or old) - let's make some plots to look at these graphically.
...
The above plot shows the raw count data (in the blue points) colored by date. The black squares represent the model estimate for each succession class (think of this as the mean correcetd for day effects).  The error bars ar 95% CIs.  Hard to see a trend I'd be confident in here.  I think the model is winning due to overfitting (the model, in the end includes 7 parameters for ~65 observations, that's not great...  I trust the graph more.)
...
Bu the above doesn't account for date yet, which we probably should do...

First a couple plots of counts by date separated by mig code and succession treatment:
...
ggplot(mig_date) +
  geom_line(aes(x = date, y = m_count, group = mig_code, 
                color = mig_code))
```
...
Let's look at the summary of our best model (spiler: it's just overfitting):
```{r}
f3 <- lmer(n ~ year_planted*mig_code  + (1|date), data = mig_counts)
summary(f3)
```
...
This model actually looks fine, but all it tells us really is that residents are more common than migrants.

## Foraging Guild

I don't think we need to even bother - if the migratory status is overfitting, a factor variable with even more categories won't perform any better.

# Preliminary Conclusions

This is a work in progress, but here's what I'm thinking for now.  There's a bump in both abundance and proportion of migrants in the 2011 treatment.  We'll need to appropriately caveat, because with the small sample size, one or a few days could be driving thism.  That said, there could be a story here.  We wouln't necessarily expect the most migrants in pasture because that'sa actually a totally different habitat type.  So you would expect the most migrants in the earliest successional stages, which is exactly what we see.  What's surprising, is that th trend appears to be nonlinear - it really onlyholds for the earliest successional habitats and after that, there is no discernable difference when compared to uncut forest.
